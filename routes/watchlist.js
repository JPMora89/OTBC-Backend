...
router.get(
  ...
  async function (req, res, next) {
    try {
      ...
      const watchlistExists = userWatchlists.some(
        (watchlist) => watchlist.watchlist_id === parseInt(watchlistId)
      );
      if (!watchlistExists) {
        return res
          .status(403)
          .json({ error: "You don't have access to this watchlist" });
      }

      // Fetch the watchlist items
      const watchlistItems = await Watchlist.getWatchlistItems(
